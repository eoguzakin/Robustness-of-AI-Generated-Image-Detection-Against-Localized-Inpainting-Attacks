...
def main():
    ...
    ap.add_argument("--out_dir", default="outputs/treering/baseline")
    args = ap.parse_args()

    P = load_yaml(args.paths)["treering"]
    ...
    dr = pd.read_csv(real_tmp)[["file", "score_treering"]].copy()
    df = pd.read_csv(fake_tmp)[["file", "score_treering"]].copy()
    dr["label"] = 0
    df["label"] = 1

    out = pd.concat([dr, df], ignore_index=True)
    out.to_csv(out_dir / "treering_baseline.csv", index=False)
    print("Wrote:", out_dir / "treering_baseline.csv")
