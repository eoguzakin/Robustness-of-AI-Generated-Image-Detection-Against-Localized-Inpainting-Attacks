...
from scripts.dimd_common import real_id_from_file, fake_id_from_file
...
def main():
    ...
    ap.add_argument("--baseline_csv", default="outputs/dimd/baseline/dimd_baseline.csv")
    ap.add_argument("--robust_dir", default="outputs/dimd/robust")
    ap.add_argument("--B", type=int, default=2000)
    args = ap.parse_args()

    B = args.B
    base = pd.read_csv(args.baseline_csv)
    scores = base["score_dimd"].to_numpy()
    ...
    reals["rid"] = reals["file"].apply(real_id_from_file)
    fakes["fid"] = fakes["file"].apply(fake_id_from_file)

    real_scores = reals["score_dimd"].to_numpy()
    fake_scores = fakes["score_dimd"].to_numpy()

    ok_real_ids = set(reals.loc[real_scores < t_opt, "rid"])
    ok_fake_ids = set(fakes.loc[fake_scores >= t_opt, "fid"])

    print("=== DIMD baseline (fixed-threshold protocol) ===")
    print(f"AP={ap100:.2f}")
    ...
    real_jobs = sorted([p.stem for p in robust_dir.glob("*_real.csv")]) + sorted([p.stem for p in robust_dir.glob("*semantic*.csv")])
    ...
    for name in fake_jobs:
        df = pd.read_csv(robust_dir / f"{name}.csv")
        df["fid"] = df["file"].apply(fake_id_from_file)

        f_attack_all = df["score_dimd"].to_numpy()
        ...
        success = (df_ok["score_dimd"].to_numpy() < t_opt).astype(int)  # evaded -> predicted real
    ...
    for name in real_jobs:
        ...
        r_attack_all = df["score_dimd"].to_numpy()
        ...
        success = (df_ok["score_dimd"].to_numpy() < t_opt).astype(int)  # passed -> predicted real
        asr = asr_ci(success, B=B)
