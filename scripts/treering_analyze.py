...
def load_baseline(path: str | Path):
    df = pd.read_csv(path)
    s = pd.to_numeric(df["score_treering"], errors="coerce").to_numpy()
...
def asr_fake(clean_fake_scores: dict, cond_fake_scores: dict, thr: float, nboot: int, alpha: float, seed: int):
    ...
    idx = np.where(clean >= thr)[0]  # baseline-correct fakes
    denom = len(idx)
    if denom == 0:
        return np.nan, 0, np.nan, np.nan, np.nan, np.nan

    evaded = (cond[idx] < thr).astype(int)
...
def main():
    ...
    ap.add_argument("--baseline_csv", default="outputs/treering/baseline/treering_baseline.csv")
    ap.add_argument("--robust_dir", default="outputs/treering/robust")
    ap.add_argument("--out_csv", default="outputs/treering/treering_summary.csv")
    ap.add_argument("--nboot", type=int, default=2000)
    ap.add_argument("--alpha", type=float, default=0.05)
    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()

    s_raw, y, f = load_baseline(args.baseline_csv)
    auc_raw = roc_auc_score(y, s_raw)
    sign = -1.0 if auc_raw < 0.5 else 1.0  # make higher=fake(watermarked)
    s = s_raw * sign
    ...
    dfb["score_treering"] = pd.to_numeric(dfb["score_treering"], errors="coerce") * sign

    base_reals = {}
    base_fakes = {}
    for fn, lb, sc in zip(dfb["file"].astype(str), dfb["label"].astype(int), dfb["score_treering"].to_numpy()):
        if lb == 0:
    ...
    for p in sorted(Path(args.robust_dir).glob("*.csv")):
        name = p.stem
        df = pd.read_csv(p)
        df["score_treering"] = pd.to_numeric(df["score_treering"], errors="coerce") * sign

        # fake splits only (we named them *_fake.csv)
        cond_fakes = {norm_fake_name(fn): float(sc) for fn, sc in zip(df["file"].astype(str), df["score_treering"].to_numpy())}

        # AUC for condition computed vs baseline reals (as in your WaRPAD recipe style)
